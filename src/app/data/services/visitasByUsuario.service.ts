...
export class VisitasByUsuario {
   ...
   getMisVisitas(){
    let filter ="?idUsuario="+ this.configService.usuario.id;
    return this.http.get(`${this.urlBase}/VisitasClientes`+ filter).pipe(
  ...
  getTecnicos(){
    return this.http.get(`${this.urlBase}/usuarios/tecnicos`).pipe(
    map((response: any)=> response.appData as any[]));
  }
